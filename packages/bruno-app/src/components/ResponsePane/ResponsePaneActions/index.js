...
import { IconDots, IconDownload, IconEraser, IconBookmark, IconCopy, IconLayoutColumns, IconLayoutRows, IconBraces, IconCode } from '@tabler/icons';
...
import ResponseGenerateInterface from '../ResponseGenerateInterface';
import ResponseGenerateReactCode from '../ResponseGenerateReactCode';
...
const ResponsePaneActions = ({ item, collection, responseSize, selectedFormat, selectedTab, data, dataBuffer }) => {
  ...
  const generateInterfaceButtonRef = useRef(null);
  const generateReactCodeButtonRef = useRef(null);
  ...
  const gqlMenuItems = [
    ...
    },
    {
      ...
      get disabled() {
        return generateInterfaceButtonRef.current?.isDisabled ?? false;
      },
      onClick: () => generateInterfaceButtonRef.current?.click()
    },
    {
      id: 'clear-response',
  ...
  const menuItems = [
    ...
    },
    {
      ...
      label: 'Generate React code',
      leftSection: IconCode,
      get disabled() {
        return generateReactCodeButtonRef.current?.isDisabled ?? false;
      },
      onClick: () => generateReactCodeButtonRef.current?.click()
    },
    {
      id: 'clear-response',
      label: 'Clear response',
  ...
  return (
    <StyledWrapper className="response-pane-actions-wrapper">
      ...
      <div className="actions-buttons flex items-center gap-[2px]">
        ...
        />
        <ResponseGenerateInterface
          ref={generateInterfaceButtonRef}
          item={item}
          data={data}
        />
        {item.type === 'http-request' && (
          <ResponseGenerateReactCode
            ref={generateReactCodeButtonRef}
            item={item}
            data={data}
          />
        )}
